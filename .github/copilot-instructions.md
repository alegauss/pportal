...
- This fork is **Windows-only**: the Linux/macOS/BSD/Android/Switch/Steam Deck ports and the
  argp-based `cli/` have been removed, and the root `CMakeLists.txt` fails on a non-Windows host.
...
- Packaging/build automation references:
  - MSYS2 deploy: `scripts/deploy-windows-msys2.sh`; installer: `scripts/chiaki-ng.iss`.
  - CI examples: `.github/workflows/build-pr.yaml`, `.github/workflows/build-release.yaml`.
...
- Do not reintroduce non-Windows branches (`Q_OS_LINUX`, `Q_OS_MACOS`, `__APPLE__`, `__SWITCH__`,
  POSIX socket/pthread paths). Windows is the only supported target.
...
- Optional integrations controlled by flags: SpeexDSP, Steam shortcut (`cpp-steam-tools`), QtWebEngine.
...
## Editing boundaries
- Prefer editing first-party code under `lib/`, `gui/`, `test/`, `cmake/`, `scripts/`.
- Avoid broad edits in vendored trees unless explicitly required: `third-party/`, `ffmpeg/`, `SDL2-*`, `sdl2-compat-*`.
