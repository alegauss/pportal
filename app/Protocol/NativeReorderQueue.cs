...
/// <summary>
/// PP23: libchiaki's reorder queue, reachable so <see cref="ReorderQueue"/> can be held against it.
///
/// It used to be what the port called. It is now the oracle: the managed queue is a transcription
/// of reorderqueue.c, and the two are driven through the same random operation sequences and
/// compared after every step. That is the only way to check a module whose whole behaviour is which
/// of four drop occasions a push takes - a packet older than the window, a duplicate, an overflow
/// at the low end and an overflow at the high end.
///
...
/// </summary>
public sealed unsafe class NativeReorderQueue : IDisposable
{
    ...
    private IntPtr Check()
        => _handle != IntPtr.Zero ? _handle : throw new ObjectDisposedException(nameof(NativeReorderQueue));
    ...
    private static void Dispatch(ulong seqNum, IntPtr elemUser, IntPtr user)
    {
        try
        {
            if (user != IntPtr.Zero && GCHandle.FromIntPtr(user).Target is NativeReorderQueue self)
