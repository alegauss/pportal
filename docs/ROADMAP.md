...
- 📋 **PP670** (deps: —) **Six managed test files call the frame-path oracles unguarded, so the flip that deletes the four files turns them red** — PP662's move is owed for the frame path: a capability export, an OfTheBuild reader, and each differential two-shape before the C leaves. → §PP670
