...
- 📋 **PP28** (deps: PP23 ⏳, PP27) **session, ctrl and streamconnection are 3977 lines of state machine with no diagram** — This is the largest single translation in the core and the one where behaviour lives in the ordering of events rather than in any function worth reading alone. → §PP28
...
- 📋 **PP30** (deps: PP23 ⏳, PP27) **forward error correction is two vendored C libraries doing Galois field arithmetic per lost packet** — the decode and encode themselves, now that PP286 has a managed field and coding matrix agreeing with jerasure's: build the submatrix, invert, multiply. → §PP30
