...
- 📋 **PP771** (deps: —) (requires: console) **the managed takion's handshake does not complete against a live console, so the run stops at the connect** — With the session's socket adopted, a live run reaches VideoReceiver and fails at ConnectTakion - the first rung a console answers rather than a test peer. → §PP771
