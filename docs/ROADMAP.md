...
- 📋 **PP330** (deps: —) **the executable, its window and the installer carry upstream's chiaki mark, and the site now ships a different one** — The mark is the first thing a download shows and the site's is this port's own, so an icon from the client being replaced tells a user the installer is that client. → §PP330
...
- ⏳ **PP293** (deps: PP297 ⏳) **session.c is 1206 lines and owns the session lifetime, and PP28 sizes it together with two files it does not resemble** — the thread itself: init, start, the connect sequence, stop and join, and the event queue a client reads. → §PP293
- 📋 **PP294** (deps: PP297 ⏳) **ctrl.c is 1481 lines of control channel and PP28 sizes it together with two files it does not resemble** — It is the longest of the three and the one with the most message types, and none of them are on the frame path so latency is not the measure. → §PP294
