...
- 📋 **PP299** (deps: —) **a console omitting system-version crashes discovery through an unguarded atoi(NULL) in the classifier** — The classifier dereferences the one discovery string the GUI does not guard, and any device on the LAN can send that reply. → §PP299
