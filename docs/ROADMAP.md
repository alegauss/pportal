...
- 📋 **PP278** (deps: —) **the corpus guard sweeps only paths starting gui\, so every drift source outside it is held by a hand-written list** — PP21 filtered on gui\ because Qt was the tree at risk; lib, test, shim and scripts paths are now named once in SelfTest.cs and nowhere asserted. → §PP278
