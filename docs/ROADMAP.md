...
- ⏳ **PP293** (deps: —) **session.c is 1182 lines and owns the session lifetime, and PP28 sizes it together with two files it does not resemble** — the lifetime itself: init, start, the connect sequence, stop and join, and the event queue a client reads. → §PP293
