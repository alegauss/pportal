...
- 📋 **PP744** (deps: —) **the pad-info order is read by a copy of the raiser sweep, still anchored on the local name PP722 widened** — PadInfoMessage.ReportOrderIn repeats EventsRaisedIn's loop with the old prefix, so one copy of one sweep was corrected and the other was not. → §PP744
