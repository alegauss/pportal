...
- 📋 **PP620** (deps: —) **five probe methods still write the STA scaffolding out inline, around bodies that hand a value back** — PP618 deleted the twenty-six reusable copies and never counted these, so the eight lines it removed are still written five times. → §PP620
