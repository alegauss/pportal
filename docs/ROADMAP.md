...
- 📋 **PP600** (deps: —) **nothing in the host constructs a session at all, so every ported connect model has no caller outside tests** — ChiakiConnectInfo is built only by the selftest and the exchange recorder; ConsoleActions decides which actions apply and nothing performs one. → §PP600
