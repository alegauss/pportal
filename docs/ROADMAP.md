...
- 📋 **PP294** (deps: PP297 ✅) **ctrl.c is 1485 lines of control channel and PP28 sizes it together with two files it does not resemble** — It is the longest of the three and the one with the most message types, and none of them are on the frame path so latency is not the measure. → §PP294
...
- 📋 **PP346** (deps: —) **the ctrl read loop adds eight to a uint32 length, and the wrap defeats the overflow check below it** — a header announcing 0xFFFFFFF8 or more makes the sum small, so the size check passes and the payload is decrypted in place over 4GB of a 512-byte buffer. → §PP346
- 📋 **PP347** (deps: —) **the rudp arm copies a message into the 512-byte ctrl buffer with no bound, and its source buffer is 520** — the only guard checks the announced payload size against the message length, which says it is well formed and nothing about whether it fits. → §PP347
- 📋 **PP348** (deps: —) **ctrl.c's ctrl_failed overwrites a recorded quit reason, which is the rule PP336 asserted the label keeps** — two writers of one field with opposite policies: the session label guards on NONE, the ctrl function assigns unconditionally on six paths. → §PP348
- 📋 **PP349** (deps: PP346) **the ctrl thread's loop is unported, and a cancelled select is its work branch rather than a failure** — the drain and the PIN send happen on CANCELED; a port reading it as an error sends nothing queued and one reading it as a timeout spins. → §PP349
- 📋 **PP350** (deps: —) **the ctrl channel owns two stop pipes for different jobs and nothing managed states which wakes what** — the notify pipe carries a stop, a queued message and a PIN into one select; the stop pipe cancels a send in progress - wiring one to both breaks a different half each way. → §PP350
