...
- ⏳ **PP8** (deps: PP4 ✅) **controller input is SDL wired into Qt events, and 950 lines of it decide what a button does** — the event loop landed in PP118; what is left is hotplug, the mappings and the DualSense specifics. → §PP8
...
- ⏳ **PP35** (deps: PP24 ✅) **5512 lines of munit tests cover the modules this port rewrites, and nothing in a managed tree runs them** — the other modules' vectors as xUnit cases: gkcrypt, rpcrypt, takion, bitstream, the reorder queue. → §PP35
...
- ⏳ **PP121** (deps: —) **eight of test/rpcrypt.c's nine recorded cases are unchecked, four of them a whole console generation** — the four registration-mode cases, which need chiaki_rpcrypt_init_regist on the shim. → §PP121
