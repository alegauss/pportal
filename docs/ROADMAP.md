...
- 📋 **PP788** (deps: —) **senkusha's nine states and the flags its waits end on have no managed model, so nothing can drive its run** — PP380 repaired the waits and modelled why each wakes; which STATE waits, and what ends it, is the half PP362 wrote for the stream connection. → §PP788
- 📋 **PP789** (deps: —) **senkusha's three measurements have no managed model, and they are the numbers the launch spec spends** — Ten pings, MTU bounds of 576 to 1454 with three retries, and a timeout derived from the round trip and clamped either side. → §PP789
- 📋 **PP790** (deps: PP788, PP789) **chiaki_senkusha_run's sequence has no managed counterpart, so the ordering that is its behaviour cannot be held** — PP295's shape one module over: a host interface is what lets a sequence be asserted without a console, and the ordering is the deliverable. → §PP790
