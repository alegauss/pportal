...
- 💭 **PP88** (deps: —) **the log sanitiser leaves the tail of a compressed IPv6 address in a file users paste into issues** — the four-group branch of its regex matches first, so the marker says redacted and the remainder is still there. → §PP88
