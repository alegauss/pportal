...
- ⏳ **PP291** (deps: —) **videoreceiver.c is the next link above the ported frame processor and no task owns it, so the chain stops there** — the bytes and the outbound half: profile switching, the flush, corrupt-frame and IDR requests. → §PP291
