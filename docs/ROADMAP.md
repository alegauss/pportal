...
- 📋 **PP770** (deps: —) **the run collapses every rung's failure into Unknown, so a live one cannot be told from any other** — A trial reached the run and it answered Unknown; the C logs which step failed and this returns one code for eight, so the next finding costs a rebuild to locate. → §PP770
