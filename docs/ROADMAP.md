...
- 📋 **PP627** (deps: PP625 ✅) **a console asking for a login PIN gets no answer, so the front door sits at Connecting for ever** — PP625 translates the events the screen has words for and drops the rest, and this is the one that needs something back before the session can go on. → §PP627
