...
- 📋 **PP36** (deps: PP24, PP22) **the only CI job is the roadkeep lint gate, so no test result can turn a branch red** — A suite nobody runs on a push goes red quietly, and Test Explorer is a local convenience rather than a gate that holds a policy in place. → §PP36
...
- 📋 **PP39** (deps: —) **the numbers the stream already computes are drawn on screen and never recorded, so the Qt build leaves no baseline** — The build that produces them is the one being deleted, so this is the only task in the roadmap whose window closes rather than moves. → §PP39
- 📋 **PP40** (deps: PP39) **glass to glass latency is not among the numbers measured, and it is the one users judge on** — Bitrate and dropped frames describe the stream, not the delay between a controller press and the picture moving, which no counter in this tree reports. → §PP40
- 📋 **PP41** (deps: PP39) **only one stage of the frame path is timed, so a slower build says which build and never which stage** — Receive, reassemble, correct, decode and present are five places a millisecond can appear, and a single delivery timestamp locates none of them. → §PP41
