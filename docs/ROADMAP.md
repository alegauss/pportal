...
- 📋 **PP294** (deps: PP297 ✅) **ctrl.c is 1710 lines of control channel and PP28 sizes it together with two files it does not resemble** — It is the longest of the three and the one with the most message types, and none of them are on the frame path so latency is not the measure. → §PP294
- 📋 **PP295** (deps: PP27 ⏳, PP297 ✅) **streamconnection.c is 1504 lines and is the last C caller of the video receiver, so every deletion below waits on it** — PP286 to PP291 ported the frame path bottom-up and none of it removed C, because this is what still calls the native receiver. → §PP295
...
- 📋 **PP393** (deps: —) **the tap emits from ctrl.c and session.c only, so the two modules PP23 still owes have nothing to replay against** — PP391 and PP392 used the two channels the capture holds; streamconnection and senkusha have no emit site, so no recording of them can exist. → §PP393
...
- 📋 **PP397** (deps: —) **a recorded BIG would publish the session id PP326 redacts on ctrl, because the secret list is keyed by ctrl type** — the recorder consults CtrlMessageSecrets for every non-session channel, and the two new ones number their messages as takion data types. → §PP397
