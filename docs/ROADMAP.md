...
- 📋 **PP696** (deps: PP707 ✅, PP753 ✅, PP755 ✅, PP758 ✅, PP759 ✅) **the frame path's deletion has no commit that edits the C, so four files stay while their ports exist** — PP623's middle step is the only one touching lib, and nobody has written this path's: session.c, the shim's wrappers, the suite's list. → §PP696
