...
- 📋 **PP304** (deps: —) **every counted claim a commit invalidates is restated by hand, from a list a red test produces after the work is done** — Three fixes in one session cost six restatements, and the tax falls hardest on the commits that explain themselves in the C. → §PP304
