...
- 📋 **PP227** (deps: PP223 ✅, PP226 ✅) **no test raises a real click, so the handler that turns one into a row is the one thing nothing exercises** — a bitmap cannot see input arrive, and the plain method the tests call is reached by a routed handler nothing drives. → §PP227
