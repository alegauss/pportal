...
- 📋 **PP294** (deps: PP297 ✅) **ctrl.c is 1713 lines of control channel and PP28 sizes it together with two files it does not resemble** — It is the longest of the three and the one with the most message types, and none of them are on the frame path so latency is not the measure. → §PP294
...
- 📋 **PP411** (deps: PP294) **the ctrl login-PIN request handler is unported, so a PIN asked for after the session id ends the session unstated** — PP408 modelled only the answer, and the request that arrives too late is the one that kills the session with CTRL_UNKNOWN. → §PP411
